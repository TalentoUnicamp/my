! function(e, t) {
    ...
    function e(t, n, o) {
        ...
        a.addEventListener("open", function(e) {
            ...
            u.onerror(e)
        }), this.addEventListener = a.addEventListener.bind(a), this.removeEventListener = a.removeEventListener.bind(a), this.dispatchEvent = a.dispatchEvent.bind(a), this.open = function(t) {
            if (r = new WebSocket(u.url, n || []), r.binaryType = this.binaryType, t) {
                if (this.maxReconnectAttempts && this.reconnectAttempts > this.maxReconnectAttempts) return
            } else a.dispatchEvent(c("connecting")), this.reconnectAttempts = 0;
            (u.debug || e.debugAll) && console.debug("ReconnectingWebSocket", "attempt-connect", u.url);
            var o = r,
                i = setTimeout(function() {
                    (u.debug || e.debugAll) && console.debug("ReconnectingWebSocket", "connection-timeout", u.url), l = !0, o.close(), l = !1
                }, u.timeoutInterval);
            r.onopen = function(n) {
                clearTimeout(i), (u.debug || e.debugAll) && console.debug("ReconnectingWebSocket", "onopen", u.url), u.protocol = r.protocol, u.readyState = WebSocket.OPEN, u.reconnectAttempts = 0;
                var o = c("open");
                o.isReconnect = t, t = !1, a.dispatchEvent(o)
            }, r.onclose = function(n) {
                if (clearTimeout(i), r = null, d) u.readyState = WebSocket.CLOSED, a.dispatchEvent(c("close"));
                else {
                    u.readyState = WebSocket.CONNECTING;
                    var o = c("connecting");
                    o.code = n.code, o.reason = n.reason, o.wasClean = n.wasClean, a.dispatchEvent(o), t || l || ((u.debug || e.debugAll) && console.debug("ReconnectingWebSocket", "onclose", u.url), a.dispatchEvent(c("close")));
                ...
                (u.debug || e.debugAll) && console.debug("ReconnectingWebSocket", "onerror", u.url, t), a.dispatchEvent(c("error"))
            }
        }, 1 == this.automaticOpen && this.open(!1), this.send = function(t) {
            if (r) return (u.debug || e.debugAll) && console.debug("ReconnectingWebSocket", "send", u.url, t), r.send(t);
...
class WebSocketBridge {
    constructor(e) {
        this.socket = null, this.streams = {}, this.default_cb = null, this.options = {
            options: e
        }
    }
    connect(e, t, n) {
        let o;
        const c = `${"https:"===window.location.protocol?"wss":"ws"}://${window.location.host}`;
        o = void 0 === e ? c : "/" == e[0] ? `${c}${e}` : e, this.socket = new ReconnectingWebSocket(o, t, n)
    }
    close(e, t) {
        this.socket.close(e, t)
    }
    listen(e) {
        this.default_cb = e, this.socket.onmessage = (e => {
            ...
            return n = t, o = null, this.default_cb ? this.default_cb(n, o) : null
        })
    }
    demultiplex(e, t) {
        this.streams[e] = t
...
class BaseSubscription {
    ...
    unsubscribe(callback) {
        ...
        if (this.debug)
            console.debug('Unsubscribed from ' + this.url);
    }
    disconnect() {
...
class ModelSubscription extends BaseSubscription {
    constructor(app, model, signal, debug) {
        ...
        this.model = model;
        this.signal = signal;
        this.url = '/ws/subscriptions/models/' + app + '/' + model + '/' + signal + '/';
    }
}
